...
inductive MyEq {α : Type u} : α → α → Type v where
  | refl (a : α) : MyEq a a
...
def ap_comp {α β γ : Type u} (f : α → β) (g : β → γ) (x y : α) (p : x ≡ y) :
  ...
  have u : P x (MyEq.refl x) := MyEq.refl (ap g (f x) (f x) (MyEq.refl (f x)))
  exact ind_eq P u y p


def ap_refl {α β : Type} (f : α → β) (x : α) : ap f x x (MyEq.refl x) ≡ MyEq.refl (f x) :=
  MyEq.refl (MyEq.refl (f x))


def transport {α : Type u} (β : (x: α) → Type v) (x y : α) : (x ≡ y) → (β x ≡ β y) :=
  by
    intro p
    let P : (y' : α) → (p' : x ≡ y') → Type v := fun y' _ => β x ≡ β y'
    exact ind_eq P (MyEq.refl (β x)) y p
